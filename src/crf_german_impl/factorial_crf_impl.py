...
class FCRF(Layer):
    """An implementation of factorial conditional random field (FCRF)."""
    ...
    def call(self, X, mask=None):
        ...
        # if mask is not None:
        #    assert K.ndim(mask) == 2, 'Input mask to FCRF must have dim 2 if not None'
    ...
    def get_config(self):
        config = {
            ...
            'unroll': self.unroll}
        base_config = super(FCRF, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))

    @property
    def loss_function(self):
        warnings.warn('FCRF.loss_function is deprecated '
    ...
    def get_negative_log_likelihood(self, y_true, X, mask):
        ...
        transp_input_tnsr = K.dot(X[0], self.kernel)
        add_prev_labels = X[1] * transp_input_tnsr
        add_prev_weights = self.activation(add_prev_labels + self.bias)
        # input_energy = self.activation(K.dot(X[0], self.kernel) + self.bias)
        input_energy = self.activation(add_prev_weights + self.bias)
        if self.use_boundary:
            input_energy = self.add_boundary_energy(input_energy, mask[0],
                                                    self.left_boundary,
                                                    self.right_boundary)
        energy = self.get_energy(y_true, input_energy, mask[0])
        print("mask--", K.int_shape(X[0])[1])
        logZ = self.get_log_normalization_constant(input_energy, mask[0],
                                                   input_length=K.int_shape(X[0])[1])
        nloglik = logZ + energy
        if mask is not None:
            nloglik = nloglik / K.sum(K.cast(mask[0], K.floatx()), 1)
        else:
            nloglik = nloglik / K.cast(K.shape(X[0])[1], K.floatx())
        return nloglik
